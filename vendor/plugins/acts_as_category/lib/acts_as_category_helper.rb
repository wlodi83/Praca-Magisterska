...
module ActsAsCategoryHelper
  ...
  def aac_tree_category(category)
    ...
    if category.ancestors_count == 0
      ...
      html_headline = '' 
      html_count = ' <span class="tree_count"> ' + h(category.articles_count.to_s)+ '</span>'
    end

    result = tag('a', {:name => anchor}) + tag('/a')
    result += "<li#{html_headline}>"
    result += '<b>' if @category == category.id
    if category.articles_count == 0 and category.ancestors_count > 0 and category.children_count == 0 then
      result += h(category.name)
    ...
    result += '</b>' if @category == category.id
    result += html_count if !category.articles_count.blank? and category.articles_count > 0
    result += '</li>'
